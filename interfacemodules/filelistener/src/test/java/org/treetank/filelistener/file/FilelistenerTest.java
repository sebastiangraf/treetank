...
public class FilelistenerTest implements IWatchCallback{
	...
	public void testMonitoring(){
		try {
			...
			File file1 = new File(new StringBuilder().append(tmpDir.getAbsolutePath()).append(File.separator).append("test1.txt").toString());
			file1.createNewFile();

			File file2 = new File(new StringBuilder().append(tmpDir.getAbsolutePath()).append(File.separator).append("test2.txt").toString());
			file2.createNewFile();
			
			File file3 = new File(new StringBuilder().append(tmpDir.getAbsolutePath()).append(File.separator).append("test3.txt").toString());
			file3.createNewFile();
 
			assertTrue(file2.delete());

			File file4 = new File(new StringBuilder().append(tmpDir.getAbsolutePath()).append(File.separator).append("test4.txt").toString());
			file4.createNewFile();
