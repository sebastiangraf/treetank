	...
	public void decryptKeyTrails(final Map<Long, byte[]> paramKeyTails) {
		...
		if (paramKeyTails.size() != 0) {
			...
			while (mIter.hasNext()) {
				...
				byte[] mChildSecretKey = mKeySelectorDb.getEntry(mapKey)
						.getSecretKey();
				byte[] mDecryptedBytes = NodeEncryption.decrypt(
						paramKeyTails.get(mapKey), mChildSecretKey);

				long mEncryptedKey = NodeEncryption
						.byteArrayToLong(mDecryptedBytes);
