...
public class DatabaseRepresentation {
				...
				if (resource.endsWith(RESTProps.COLEND)) {
					...
					if (tnkFile.exists()) {
						...
						final String saveName = newCol.getName()
								+ File.separatorChar + (new Date().getTime());
						shred(input, saveName);
					}
				}
			} catch (final TreetankException exce) {
	...
	public void deleteResource(final String resourceName)
		...
		synchronized (resourceName) {
			if (resourceName.endsWith(RESTProps.COLEND)) {
				isDeleted = deleteCompleteDirectory(new File(
						RESTProps.STOREDBPATH + File.separatorChar
								+ resourceName));
				if (!isDeleted) {
				...
				if (!isDeleted) {
					throw new JaxRxException(500,
							"Deletion could not be performed");
				}
			}
		}
	}
	...
	private final OutputStream serialize(final String resource,
			...
			throws IOException, JaxRxException, TreetankException {
	...
	public void revertToRevision(final String resourceName,
			final long backToRevision) throws JaxRxException, TreetankException {
		final StringBuilder tnkFileName = new StringBuilder(
