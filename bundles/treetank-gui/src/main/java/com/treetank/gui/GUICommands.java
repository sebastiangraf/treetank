...
public enum GUICommands implements GUICommand {
    ...
    private static void setViews(final GUI gui, final File file) {
        try {
            ...
            for (final AdjustmentListener listener : bar
            ...
            final XMLSerializer serializer = new XMLSerializerBuilder(new DescendantAxis(rtx), out)
                    .build();
            serializer.call();
            ...
            tree.addTreeSelectionListener(new TreeSelectionListener() {
                public void valueChanged(final TreeSelectionEvent e) {
                    if (e.getNewLeadSelectionPath() != null
                         ...
                         * a single selection.
                         */
                        final IItem node = (IItem) e.getNewLeadSelectionPath()
                                .getLastPathComponent(); // tree.getLastSelectedPathComponent();
                        out = new ByteArrayOutputStream();
                        ...
                        try {
                            ...
                            case ROOT_KIND:
                                rtx.moveTo(nodeKey);
                                new XMLSerializerBuilder(new DescendantAxis(rtx), out).build()
                                        .call();
                                break;
                            case ELEMENT_KIND:
                                rtx.moveTo(nodeKey);
                            ...
                            case TEXT_KIND:
                                ...
                                break;
                            case ATTRIBUTE_KIND:
                                // Move transaction to parent of given attribute
                                // node.
                                rtx.moveTo(node.getParentKey());
                                final long aNodeKey = node.getNodeKey();
                                for (int i = 0, attsCount = ((ElementNode) rtx
                                        .getNode()).getAttributeCount(); i < attsCount; i++) {
                                ...
                                // Display value.
                                final String attPrefix = rtx
                                        .getQNameOfCurrentNode().getPrefix();
                                final QName attQName = rtx
                                        .getQNameOfCurrentNode();

                                if (attPrefix == null || attPrefix == "") {
                                    out.write((attQName.getLocalPart() + "='"
                                            + rtx.getValueOfCurrentNode() + "'")
                                            .getBytes());
                                } else {
                                    out.write((attPrefix + ":"
    ...
    public static void text(final GUI gui, final JTextArea xmlPane,
            final boolean init) {
        ...
        final int lineHeight = xmlPane.getFontMetrics(xmlPane.getFont())
                .getHeight();
        ...
        for (int i = startPos == 0 ? startPos : startPos + 1; i < text.length
                && lineChanges >= 0 && startPos + 1 != text.length; i++) {
            final char character = text[i];

            // Increment rowsSize?
            if (indexSepChar < NL.length()
                    && character == NL.charAt(indexSepChar)) {
                if (indexSepChar == NL.length() - 1) {
                    rowsSize += lineHeight;
                } else {
                    indexSepChar++;
        ...
        if (lineChanges >= 0 && startPos + 1 <= text.length) {
            if (init) {
                ...
                final int newCaretPos = caretPos + lineChanges
                        * xmlPane.getColumns();
        ...
        if (fc.showOpenDialog(gui) == JFileChooser.APPROVE_OPTION) {
            ...
            if (fc.showSaveDialog(gui) == JFileChooser.APPROVE_OPTION) {
                ...
                try {
                    ...
                    final ISession session = database.getSession();
                    final IWriteTransaction wtx = session
                            .beginWriteTransaction();
                    final XMLEventReader reader = XMLShredder
                            .createReader(source);
                    if (updateOnly) {
                        final XMLShredder shredder = new XMLUpdateShredder(wtx,
                                reader, true);
                        shredder.call();
                    } else {
