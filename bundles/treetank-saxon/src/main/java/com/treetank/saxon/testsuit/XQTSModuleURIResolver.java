...
public class XQTSModuleURIResolver implements ModuleURIResolver {
    ...
    public StreamSource[] resolve(final String moduleURI, final String baseURI, final String[] locations) throws XPathException {
        final NamePool pool = testCase.getNamePool();
        final int moduleNC = pool.allocate("", "http://www.w3.org/2005/02/query-test-XQTSCatalog", "module");
        final int namespaceNC = pool.allocate("", "", "namespace");
        final AxisIterator iter = testCase.iterateAxis(Axis.CHILD, new NameTest(Type.ELEMENT, moduleNC, pool));
        final List catalogLocations = new ArrayList(5);
        while (true) {
        ...
        if (catalogLocations.isEmpty()) {
            ...
            final StandardModuleURIResolver r = StandardModuleURIResolver.getInstance();
