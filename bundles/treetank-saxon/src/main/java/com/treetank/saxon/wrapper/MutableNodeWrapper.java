...
public class MutableNodeWrapper extends NodeWrapper implements MutableNodeInfo {
	 ...
	 */
	@Override
	public void addAttribute(final int nameCode, final int typeCode,
			final CharSequence value, final int properties) {
		if (mWTX.getNode().isElement()) {
			...
			try {
				mWTX.insertAttribute(mWTX.getQNameOfCurrentNode(),
						(String) value);
	 ...
	 */
	@Override
	public void insertChildren(NodeInfo[] source, boolean atStart,
			boolean inherit) {
		if (mWTX.getNode().isDocumentRoot() || mWTX.getNode().isElement()) {
			...
			for (final NodeInfo node : source) {
				try {
					if (first) {
						mWTX.insertElementAsFirstChild(new QName(node.getURI(),
								node.getLocalPart(), node.getPrefix()));
						first = false;
					} else {
						mWTX.insertElementAsRightSibling(new QName(node
								.getURI(), node.getLocalPart(), node
								.getPrefix()));
					}
				} catch (TreetankException e) {
					LOGGER.error("Insertion of element failed: "
							+ e.getMessage(), e);
				}
			}
	 ...
	 */
	@Override
	public void insertSiblings(NodeInfo[] source, boolean before,
			boolean inherit) {
		if (before) {
			mWTX.moveToParent();
			...
			for (final NodeInfo node : source) {
				try {
					...
					LOGGER.error("Inserting element failed: " + e.getMessage(),
							e);
				}
			}
	...
	public void removeAttribute(final NodeInfo attribute) {
		if (mWTX.getNode().isAttribute()
				&& ENodes.ATTRIBUTE_KIND.getNodeIdentifier() == attribute
						.getNodeKind()) {

			for (int i = 0, attCount = mWTX.getNode().getAttributeCount(); i < attCount; i++) {
				mWTX.moveToAttribute(i);
				try {
					if (mWTX.getQNameOfCurrentNode().equals(
							attribute.getDisplayName())) {
						mWTX.remove();
					}
				} catch (TreetankException e) {
					LOGGER.error(
							"Removing attribute failed: " + e.getMessage(), e);
				}
				mWTX.moveTo(mKey);
			}
		}
