...
public class NodeWrapper implements NodeInfo, VirtualNode, SiblingCountingNode {
  ...
  public NodeInfo getParent() {
    ...
    if (mRTX.getNode().hasParent()) {
      ...
      final NodeInfo mParent =
          makeWrapper(mDocWrapper, mRTX.getNode().getParentKey());
      return mParent;
    } else {
      return null;
    }
  ...
  public boolean hasChildNodes() {
    ...
    if (node instanceof IStructuralNode
        && ((IStructuralNode) mRTX.getNode()).getChildCount() > 0) {
      hasChildNodes = true;
    }
  ...
  public int getSiblingPosition() {
    ...
    if (node instanceof IStructuralNode) {
      while (((IStructuralNode) mRTX.getNode()).hasLeftSibling()) {
