...
public class NodeWrapper implements NodeInfo, VirtualNode, SiblingCountingNode {
	 ...
	 * @return nodeWrapper.
	 */
	protected NodeWrapper makeWrapper(final ISession session,
			final DocumentWrapper docWrapper, final long key) {
	...
	public int[] getDeclaredNamespaces(final int[] buffer) {
		...
		if (nodeKind == ENodes.ELEMENT_KIND) {
			...
			if (count == 0) {
				...
				int[] result = (buffer == null || count > buffer.length ? new int[count]
						: buffer);
	...
	public NodeInfo getParent() {
		if (mRTX.getNode().hasParent()) {
			// Parent transaction.
			final NodeInfo mParent = makeWrapper(mSession, mDocWrapper, mRTX
