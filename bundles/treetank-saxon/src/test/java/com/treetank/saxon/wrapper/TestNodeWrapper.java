...
public class TestNodeWrapper {
  ...
  public void beforeMethod() throws TreetankException {
    ...
    final IWriteTransaction wtx =
        databaseTest.getSession().beginWriteTransaction();
  ...
  public void testCompareOrder() throws XPathException, TreetankException {
    ...
    NodeWrapper node =
        (NodeWrapper) new DocumentWrapper(database, config).wrap();
    NodeWrapper other =
        (NodeWrapper) new DocumentWrapper(databaseTest, config).wrap(3);
  ...
  public void testGetAttributeValue() {
    final Processor proc = new Processor(false);
    node =
        (NodeWrapper) new DocumentWrapper(databaseTest, proc
  ...
  public void testGetBaseURI() throws Exception {
    ...
    assertEquals(TestHelper.PATHS.PATH1.getFile().getAbsolutePath(), node
        .getBaseURI());
    ...
    final NodeWrapper doc =
        (NodeWrapper) new DocumentWrapper(database, proc
            .getUnderlyingConfiguration()).wrap();
  ...
  public void testGetStringValueCS() {
    ...
    final NodeWrapper attrib =
        (NodeWrapper) node.iterateAxis(Axis.ATTRIBUTE).next();
