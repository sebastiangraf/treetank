...
public final class Database implements IDatabase {
     ...
     * Private constructor
     */
    private Database(final DatabaseConfiguration databaseConf, final SessionConfiguration sessionConf)
    ...
    /**
     * Creating a database. This includes loading the database configurations, building up the structure and
     * preparing everything for login.
     * 
     * @param conf which are used for the database
     * @return true of creation is valid, false otherwise
     * @throws TreetankIOException if something odd happens within the creation process.
     */
    public synchronized static boolean createDatabase(final DatabaseConfiguration conf)
            ...
            if (file.exists()) {
                ...
                if (returnVal) {
                    for (EStoragePaths paths : EStoragePaths.values()) {
                        final File toCreate = new File(conf.getFile(), paths.getFile().getName());
                        if (paths.isFolder()) {
    ...
    /**
     * Truncate a database. This deletes all relevant data. If there are existing sessions against this
     * database, the method returns null.
     * 
     * @param file the database at this path should be deleted.
    ...
    /**
     * Open database. A database can be opened only once. Afterwards the singleton instance bound to the File
     * is given back.
     * 
     * @param file where the database is located
     * @return {@link IDatabase} instance.
     * @throws TreetankException if something odd happens
     */
    public synchronized static IDatabase openDatabase(final File file) throws TreetankException {
     ...
     * is given back.
     * 
     * @param file where the database is located sessionConf a {@link SessionConfiguration} object to set up
     *            the session
     * @return {@link IDatabase} instance.
     * @throws TreetankException if something odd happens
     */
    public synchronized static IDatabase openDatabase(final File file, final SessionConfiguration sessionConf)
        throws TreetankException {
        if (!file.exists() && !createDatabase(new DatabaseConfiguration(file))) {
            throw new TreetankUsageException("DB could not be created at location", file.toString());
        }
        IDatabase database = DATABASEMAP.putIfAbsent(file, new Database(new DatabaseConfiguration(file),
                                                                        sessionConf));
        if (database == null) {
     ...
     * 
     * @param file where the database should be closed
     * @throws TreetankException if something weird happens while closing
     */
    public synchronized static void forceCloseDatabase(final File file) throws TreetankException {
    ...
    /**
     * Closing a database. All {@link ISession} instances within this database are closed.
     * 
     * @throws TreetankException if close is not successful.
     */
        ...
        if (storedVersions[0] < versions[0]) {
            ...
            if (storedVersions[1] < versions[1]) {
                throw new TreetankUsageException("Version Minor expected:",
                                                 Integer.toString(storedVersions[1]), "but was",
                                                 Integer.toString(versions[1]));
            } else {
