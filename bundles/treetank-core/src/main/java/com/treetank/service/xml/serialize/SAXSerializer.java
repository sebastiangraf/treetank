...
public final class SAXSerializer extends AbsSerializer implements XMLReader {
  ...
  protected void emitStartElement(final IReadTransaction rtx) {
    ...
    try {
      mContHandler.endElement(URI, qName.getLocalPart(), WriteTransactionState
          .buildName(qName));
  ...
  private void generateElement(final IReadTransaction rtx) {
    ...
    for (int i = 0, namesCount =
        ((ElementNode) rtx.getNode()).getNamespaceCount(); i < namesCount; i++) {
      ...
      if (rtx.nameForKey(rtx.getNode().getNameKey()).length() == 0) {
        ...
        atts.addAttribute(URI, "xmlns", "xmlns:"
            + rtx.getQNameOfCurrentNode().getLocalPart(), "CDATA", URI);
      }
      rtx.moveTo(key);
    }
    ...
    for (int i = 0, attCount =
        ((ElementNode) rtx.getNode()).getAttributeCount(); i < attCount; i++) {
      rtx.moveToAttribute(i);
      final String URI = rtx.nameForKey(rtx.getNode().getURIKey());
      final QName qName = rtx.getQNameOfCurrentNode();
      atts.addAttribute(URI, qName.getLocalPart(), WriteTransactionState
          .buildName(qName), rtx.getTypeOfCurrentNode(), rtx
  ...
  private void generateText(final IReadTransaction rtx) {
    try {
      mContHandler.characters(rtx.getValueOfCurrentNode().toCharArray(), 0, rtx
          .getValueOfCurrentNode()
          .length());
    } catch (final SAXException e) {
      LOGGER.error(e.getMessage(), e);
