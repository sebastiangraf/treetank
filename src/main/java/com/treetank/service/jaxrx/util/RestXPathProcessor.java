...
public class RestXPathProcessor {
    ...
    private final transient WorkerHelper workerHelper;
    /**
    ...
    /**
     * This constructor instantiates this class with the reference of
     * {@link WorkerHelper}.
     * 
     * @param worker
     *            The {@link WorkerHelper} reference.
     */
    public RestXPathProcessor(final WorkerHelper worker) {
        workerHelper = worker;
    }
    ...
    public OutputStream getXpathResource(final String resourceName,
            ...
            throws IOException, TreetankException {
        ...
        if (resourceName.endsWith(RESTProps.COLEND)) {
            ...
            if (wrapResult) {
                ...
                for (final File aTNK : tnks) {
                    doXPathRes(aTNK, revision, output, nodeid, xpath);
                }
            ...
            if (workerHelper.checkExistingResource(dbFile)) {
                ...
                if (wrapResult) {
                    output.write(beginResult.getBytes());
                    doXPathRes(dbFile, revision, output, nodeid, xpath);
                    output.write(endResult.getBytes());
                } else {
                    doXPathRes(dbFile, revision, output, nodeid, xpath);
                }
    ...
    public void getXpathResource(final File dbFile, final long rId,
            ...
            final OutputStream output, final boolean doWrap)
            throws TreetankException {

        final String uncheckedQuery = query;
        String checkedQuery;
        if (uncheckedQuery.matches(".*//.*")) {
            checkedQuery = uncheckedQuery.replace("//", ".//");
        ...
        try {
            ...
            if (exist) {
                final IAxis axis = new XPathAxis(rtx, checkedQuery);
                if (doWrap) {
                    output.write(beginResult.getBytes());
                    for (final long key : axis) {
                        if (key >= 0) {
                            workerHelper.serializeXML(rtx, output, false,
                                    doNodeId).call();
                        } else {
     ...
     * @throws TreetankException
     */
    private void doXPathRes(final File dbFile, final Long revision,
