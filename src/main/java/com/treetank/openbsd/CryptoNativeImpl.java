...
public final class CryptoNativeImpl implements ICrypto {
	...
	private static final BlockingQueue coreQueue = new ArrayBlockingQueue(
			CORE_COUNT);
	...
	static {
		try {
			...
			throw new RuntimeException(
					"Could not allocate native compression cores.");
		}
	}
	...
	public final short crypt(final short length, final IByteBuffer buffer) {
		...
		try {
			...
			result = syscall(tank, (byte) (command | core), length,
					((ByteBufferNativeImpl) buffer).getAddress());
		} catch (Exception e) {
			return ERROR;
		} finally {
			try {
	 ...
	 * 
	 * @param tank
	 *            TreeTank identifier in [1, ..., 256].
	 * @param operation
	 *            Operation identifier in [1, ..., 256].
	 *            <code>(operation & 0xF)</code> = command in [1, ..., 16].
	 *            <code>(operation >> 0x4)</code> = core in [1, ..., 16].
	 * @param length
	 *            Length of input buffer in [1, ..., 32767].
	 * @param buffer
	 *            Direct data exchange buffer != NULL.
	 * @return Length of output buffer in [1, ..., 32767] or error in [0].
	 */
	private final native short syscall(final byte tank, final byte operation,
			final short length, final int address);
