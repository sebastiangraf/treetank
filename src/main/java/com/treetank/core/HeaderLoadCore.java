 ...
 * 
 * $Id: HeaderLoadCore.java 4241 2008-07-03 14:43:08Z kramis $
 */
...
public final class HeaderLoadCore implements IHeaderLoadCore {
	...
	public final Configuration load() {
		...
		try {
			...
			for (int i = 0; i < 480; i++) {
				if ((header1[i] != header2[i]) || (header1[i] != header3[i])
						|| (header1[i] != header4[i])) {
					throw new IllegalStateException("Header of '" + mDevice
							+ "' do not match.");
				}
			}

			for (int i = 480; i < 512; i++) {
				if ((header1[i] != 0) || (header2[i] != 0) || (header3[i] != 0)
						|| (header4[i] != 0)) {
					throw new IllegalStateException("Header of '" + mDevice
			...
			throw new RuntimeException("HeaderLoadCore " + "could not load '"
					+ mDevice + "' due to: " + e.toString());
		}
