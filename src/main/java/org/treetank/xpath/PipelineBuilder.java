...
import org.treetank.xpath.comparators.GeneralComp;
import org.treetank.xpath.comparators.NodeComp;
import org.treetank.xpath.comparators.ValueComp;
import org.treetank.xpath.expr.AndExpr;
import org.treetank.xpath.expr.CastExpr;
import org.treetank.xpath.expr.CastableExpr;
import org.treetank.xpath.expr.EveryExpr;
import org.treetank.xpath.expr.ExceptAxis;
import org.treetank.xpath.expr.ForAxis;
import org.treetank.xpath.expr.IfAxis;
import org.treetank.xpath.expr.InstanceOfExpr;
import org.treetank.xpath.expr.IntersectAxis;
import org.treetank.xpath.expr.LiteralExpr;
import org.treetank.xpath.expr.OrExpr;
import org.treetank.xpath.expr.RangeAxis;
import org.treetank.xpath.expr.SequenceAxis;
import org.treetank.xpath.expr.SomeExpr;
...
import org.treetank.xpath.operators.AddOpAxis;
import org.treetank.xpath.operators.DivOpAxis;
import org.treetank.xpath.operators.IDivOpAxis;
import org.treetank.xpath.operators.ModOpAxis;
...
public final class PipelineBuilder {
  ...
  /**
   * Maps a function name to the ordinal value of the function in the enum Func.
   */
  ...
  public void finishExpr(final IReadTransaction transaction, final int num) {
    ...
    // sequence, so the number of singleExpressions in the sequence and the size
  ...
  public void addExpressionSingle() {
    ...
    // expression, therefore a new expression chain is build and added to the
    // stack.
  ...
  public void addUnionExpression(final IReadTransaction transaction) {
    ...
    getExpression().add(
        new DupFilterAxis(transaction, new UnionAxis(transaction, operand1,
            operand2)));
  }
  ...
  public void addIntExcExpression(final IReadTransaction transaction,
    ...
    final IAxis axis = isIntersect ? new IntersectAxis(rtx, operand1, operand2)
  ...
  public void addPredicate(final IReadTransaction transaction) {
    ...
    if (predicate instanceof LiteralExpr) {
      ...
      int type = transaction.getTypeKey();
...
//        getExpression().add(
//            new PosFilter(transaction, (int) Double.parseDouble(transaction
//                .getValue())));
//        return; // TODO: YES! it is dirty!
        ...
        // IAxis function = new FNPosition(transaction, new ArrayList<IAxis>(),
  ...
  public void addFunction(final IReadTransaction transaction,
    ...
    // arguments are stored on the stack in reverse order -> invert arg order
    for (int i = 0; i < num; i++) {
    ...
    try {
      ...
      final IAxis axis = (IAxis) cons.newInstance(transaction, args, min, max,
          returnType);
