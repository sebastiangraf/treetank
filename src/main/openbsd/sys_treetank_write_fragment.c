  ...
  u_int8_t  *bufferPtr)
{
  ...
  if (*lengthPtr < TT_REFERENCE_LENGTH) {
    ...
    printf("ERROR(sys_treetank_write_fragment.c): Invalid lengthPtr argument.\n");
    goto finish;
  }
  ...
  if (sys_treetank_compression(
    ...
    syscall = TT_SYSCALL_FAILURE;
    printf("ERROR(sys_treetank_write_fragment.c): Could not perform compression.\n");
    goto finish;
  }
  ...
  if (sys_treetank_encryption(
      ...
      bufferPtr) != TT_SYSCALL_SUCCESS) {
    syscall = TT_SYSCALL_FAILURE;
    printf("ERROR(sys_treetank_write_frament.c): Could not perform encryption.\n");
    goto finish;
  }
  ...
  if (sys_treetank_authentication(
      ...
      bufferPtr) != TT_SYSCALL_SUCCESS) {
    syscall = TT_SYSCALL_FAILURE;
    printf("ERROR(sys_treetank_write_fragment.c): Could not perform authentication.\n");
    goto finish;
  }
