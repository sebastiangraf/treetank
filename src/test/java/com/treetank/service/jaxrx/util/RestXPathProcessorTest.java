...
public class RestXPathProcessorTest {
    ...
    /**
     * Instances param jax result static variable.
     */
    ...
    public static void setUpGlobal() throws TreetankException {
        ...
        new TreeTank().shred(xmlInput, RESOURCENAME);
    }
    ...
    public final void testGetXpathResourceStringStringBooleanLongOutputStreamBoolean()
        ...
        assertEquals("test for items size of authors is 12", 12, aNodes
                .getLength());
        final NodeList rNodes = resultDoc.getElementsByTagName(PARAMJAXRES);
        assertEquals("test for existence of result element is 1", 1, rNodes
                .getLength());
        ...
        rxProcessor.getXpathResource(RESOURCENAME, xPath, withNodeIds, null,
                output, true);
        xmlInput = new ByteArrayInputStream(((ByteArrayOutputStream) output)
                .toByteArray());
        resultDoc = xmlDocument(xmlInput);
        final NodeList b2Nodes = resultDoc.getElementsByTagName(PARAMBOOKS);
        assertEquals("test for items size of books is 0", 0, b2Nodes
                .getLength());
        ...
        assertEquals("test for existence of result element", 1, r2Nodes
                .getLength());
    ...
    public final void testGetXpathResourceFileLongStringBooleanLongOutputStreamBoolean()
        ...
        final NodeList aNodes = resultDoc.getElementsByTagName(PARAMAUTHOR);
        assertEquals("test for items size of authors is 2", 2, aNodes
                .getLength());
        final NodeList rNodes = resultDoc.getElementsByTagName(PARAMJAXRES);
        assertEquals("test for item of result element is 1", 1, rNodes
                .getLength());
        final NodeList iNodes = resultDoc.getElementsByTagName(PARAMJRESTSEQ);
