...
public class BucketWriteTrxTest {
	...
	public void setUp() throws Exception {
		...
		final ResourceConfiguration config = mResourceConfig
				.create(StandardSettings.getProps(CoreTestHelper.PATHS.PATH1
	...
	public void testRevision() throws TTException {
		...
		assertEquals(mHolder.getSession().getMostRecentVersion() + 1,
				wtx.getRevision());
		wtx.close();
	}
	...
	/**
	 * Test method for
	 * {@link org.treetank.access.BucketWriteTrx#getMetaBucket()}.
	 * 
	 * @throws TTException
	 */
	@Test
	public void testGetMetaBucket() throws TTException {
		List<List<Map.Entry<DumbKey, DumbValue>>> meta = CoreTestHelper
				.createTestMeta(mHolder);
	...
	/**
	 * Test method for
	 * {@link org.treetank.access.BucketWriteTrx#setData(org.treetank.api.IData)}
	 * .
	 * 
	 * @throws TTException
	 */
	...
	public void testSetData() throws TTException {
		...
		for (int j = 0; j < versionToWrite; j++) {
			for (int i = 0; i < elementsToSet; i++) {
				assertEquals(new StringBuilder("Datas differ in version ")
						.append(j).append(" and data ").append(i).toString(),
						datas.get(i), wtx.getData(i));
		...
		final IBucketReadTrx rtx = mHolder.getSession().beginBucketRtx(
	...
	/**
	 * Test method for
	 * {@link org.treetank.access.BucketWriteTrx#removeData(org.treetank.api.IData)}
	 * .
	 * 
	 * @throws TTException
	 */
	...
	public void testRemoveData() throws TTException {
		...
		for (int i = 0; i < revisions; i++) {
			for (int j = 0; j < elementsDeleted; j++) {
				int nextElementKey = (int) Math.abs((CoreTestHelper.random
						.nextLong() + 1) % list.size());
	...
	public void testCloseAndIsClosed() throws TTException {
		...
		BucketReadTrxTest.testClose(mHolder.getStorage(), mHolder.getSession(),
				rtx);
	}
	...
	public static void main(String[] args) {
		try {
			...
			final Properties props = StandardSettings.getProps(
					CoreTestHelper.PATHS.PATH1.getFile().getAbsolutePath(),
					CoreTestHelper.RESOURCENAME);
			final IBackend backend = new JCloudsStorage(props, dataFac,
					metaFac, new ByteHandlerPipeline());

			ResourceConfiguration config = new ResourceConfiguration(props,
					backend, revision, dataFac, metaFac);
