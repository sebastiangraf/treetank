...
public class IRevisioningTest {
	 ...
	 * . This test just takes two versions and checks if the version-counter is
	...
	public void testCombineBucketsForModification(
			Class<IRevisioning> pRevisioningClass, IRevisioning[] pRevisioning,
			Class<IRevisionChecker> pRevisionCheckerClass,
			IRevisionChecker[] pRevisionChecker,
			Class<IDataBucketGenerator> pDataGeneratorClass,
		...
		for (int i = 0; i < pRevisioning.length; i++) {
			// ...check if revision is not SlidingSnapshot (since
			...
			if (!(pRevisioning[i] instanceof SlidingSnapshot)) {
				...
				final DataBucket[] buckets = pDataGenerator[i]
						.generateDataBuckets();
				// ..recombine them...
				final LogValue bucket = pRevisioning[i]
						.combineBucketsForModification(buckets.length, 0,
								buckets, true);
				// ...and check them suitable to the versioning approach
				pRevisionChecker[i].checkCompleteBucketsForModification(bucket,
						buckets, true);
			}
		}
		...
		for (int i = 0; i < pRevisioning.length; i++) {
			...
			if (!(pRevisioning[i] instanceof FullDump)) {
				...
				// ..recombine them...
				final LogValue bucket = pRevisioning[i]
						.combineBucketsForModification(buckets.length - 1, 0,
								buckets, false);
				// ...and check them suitable to the versioning approach
				pRevisionChecker[i].checkCompleteBucketsForModification(bucket,
						buckets, false);
	 ...
	 */
	@Test(dataProvider = "instantiateVersioning")
	public void testCombineBuckets(Class<IRevisioning> pRevisioningClass,
	...
	/**
	 * Providing different implementations of the {@link IRevisioning} as
	 * Dataprovider to the test class.
	 * 
	 * @return different classes of the {@link IRevisioning} and
	 *         <code>IRevisionChecker</code>
	...
	public Object[][] instantiateVersioning() throws TTByteHandleException {
		...
		Object[][] returnVal = { {
				IRevisioning.class,
				new IRevisioning[] { new FullDump(), new Incremental(),
						new Differential(), new SlidingSnapshot() },
				IRevisionChecker.class,
				new IRevisionChecker[] {
						// Checker for FullDump
						new IRevisionChecker() {
							@Override
							public void checkCompleteBuckets(
									DataBucket pComplete,
									DataBucket[] pFragments) {
								...
								for (int i = 0; i < pComplete.getDatas().length; i++) {
									assertEquals("Check for FullDump failed.",
											pFragments[0].getData(i),
											pComplete.getData(i));
								}
							}

							@Override
							public void checkCompleteBucketsForModification(
									LogValue pComplete,
								...
								// complete dump consists out of the last
								// version within the FullDump
								DataBucket complete = (DataBucket) pComplete
										.getComplete();
								DataBucket modified = (DataBucket) pComplete
										.getModified();
						...
						new IRevisionChecker() {
							...
							public void checkCompleteBuckets(
								...
								for (int i = 0; i < pFragments.length - 1; i++) {
									for (j = i * 2; j < (i * 2) + 2; j++) {
										assertEquals(
												"Check for Incremental failed.",
												pFragments[i].getData(j),
												pComplete.getData(j));
									}
								}
								...
								for (; j < pComplete.getDatas().length; j++) {
									assertEquals(
											"Check for Incremental failed.",
											pFragments[pFragments.length - 1]
													.getData(j),
											pComplete.getData(j));
								}
							}
						...
						, new IRevisionChecker() {
							...
							public void checkCompleteBuckets(
								...
								for (j = 0; j < 32; j++) {
									assertEquals(
											"Check for Differential failed.",
											pFragments[0].getData(j),
											pComplete.getData(j));
								}
							...
							public void checkCompleteBuckets(
								...
								for (int i = 0; i < pFragments.length; i++) {
									for (int j = i * 2; j < (i * 2) + 2; j++) {
										assertEquals(
												"Check for Sliding Snapshot failed.",
												pFragments[i].getData(j),
												pComplete.getData(j));
									}
								}
						...
						} }, IDataBucketGenerator.class,
				new IDataBucketGenerator[] {
						// Checker for FullDump
						new IDataBucketGenerator() {
							@Override
							public DataBucket[] generateDataBuckets() {
								DataBucket[] returnVal = { getDataBucket(0,
										IConstants.CONTENT_COUNT, 0, -1) };
								return returnVal;
							}
						},
						// Checker for Incremental
	...
	interface IRevisionChecker {
		...
		void checkCompleteBucketsForModification(LogValue pComplete,
				DataBucket[] pFragments, boolean fullDump);
	}
